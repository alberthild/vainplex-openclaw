...
function aggregateMatches(matches: MatchedPolicy[]): EvalResult {
  ...
  for (const m of matches) {
    ...
    } else if (m.effect.action === "2fa") {
      has2fa = true;
      if (!twoFaReason) twoFaReason = "reason" in m.effect ? (m.effect.reason ?? "") : "";
    } else if (m.effect.action === "audit") {
      hasAudit = true;
    }
  }

  // deny > 2fa > audit > allow
  if (hasDeny) {
    return { action: "deny", reason: denyReason || "Denied by governance policy", matches };
  }
  if (has2fa) {
    return { action: "2fa", reason: twoFaReason || "Requires 2FA approval", matches };
  }
  if (hasAudit) {
