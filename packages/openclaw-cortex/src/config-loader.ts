...
function readJsonFile(
  ...
  try {
    ...
    logger.warn(
      `[cortex] Failed to read config file ${path}: ${e instanceof Error ? e.message : String(e)}`,
    );
    return null;
  }
}
...
function bootstrapConfig(
  ...
  try {
    ...
    logger.warn(
      `[cortex] Failed to write default config: ${e instanceof Error ? e.message : String(e)}`,
    );
    return null;
  }
}
